package egsa.app.view

import egsa.app.controller.Main
import javafx.application.Platform
import javafx.beans.property.SimpleIntegerProperty
import javafx.beans.property.SimpleStringProperty
import javafx.stage.FileChooser
import simplicityAlgorithm
import tornadofx.*
import java.io.File
            ...
            hbox(spacing = 10) {
                button("encrypt") {
                    action {
                        ...
                        runAsyncWithProgress {
                            measureTimeMillis {
                                controller.sign(File(input.value), File(output.value))
                ...
                button("decrypt") {
                    action {
                        ...
                        runAsyncWithProgress {
                            measureTimeMillis {
                                controller.checkSignature(File(input.value), File(output.value))
                            }.let { handleTime.value = "$it ms" }
                        }
                    }
                }
